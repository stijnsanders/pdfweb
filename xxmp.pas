...
begin
  Result:=TXxmpdfweb.Create(AProjectName);
end;
...
function TXxmpdfweb.LoadPage(Context: IXxmContext; const Address: WideString): IXxmFragment;
var
  verb:WideString;
begin
  inherited;
...
function TXxmpdfweb.LoadFragment(Context: IXxmContext; const Address, RelativeTo: WideString): IXxmFragment;
begin
  Result:=XxmFragmentRegistry.GetFragment(Self,Address,RelativeTo);
end;

procedure TXxmpdfweb.UnloadFragment(Fragment: IXxmFragment);
begin
  inherited;
